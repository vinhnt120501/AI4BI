...
import os
import time
from db import compute_data_summary
...
def build_reply_contents(question: str, columns: list, rows: list,
    ...
    max_rows = int(os.getenv("REPLY_MAX_ROWS", "80"))
    sample_rows = rows[:max_rows]
    data_text = json.dumps({"columns": columns, "rows": sample_rows}, ensure_ascii=False)
    summary_text = json.dumps(data_summary, ensure_ascii=False)
    ...
    contents += f"Data Summary (TOÀN BỘ {len(rows)} rows):\n{summary_text}\n\n"
    contents += f"Dữ liệu ({len(sample_rows)}/{len(rows)} rows):\n{data_text}"

    if additional_data:
        for i, extra in enumerate(additional_data, 1):
            extra_summary = compute_data_summary(extra["columns"], extra["rows"])
            extra_sample = extra["rows"][:30]
            contents += f"\n\n--- Dữ liệu bổ sung #{i} (Lý do: {extra.get('reason', 'drill-down')}) ---\n"
            contents += f"SQL: {extra.get('sql', '')}\n"
            contents += f"Summary: {json.dumps(extra_summary, ensure_ascii=False)}\n"
            contents += f"Data ({len(extra_sample)}/{len(extra['rows'])} rows): "
            contents += json.dumps({"columns": extra["columns"], "rows": extra_sample}, ensure_ascii=False)
    ...
    return {
        ...
        "counts": {
            ...
            "summary": count_tokens(summary_text),
