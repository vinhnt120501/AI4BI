...
import { Edit3, PanelRightClose, PanelRightOpen, Plus } from 'lucide-react';
import AnalysisPage from '@/components/workspace/pages/AnalysisPage';
import ExplorePage from '@/components/workspace/pages/ExplorePage';
import OverviewPage from '@/components/workspace/pages/OverviewPage';
import type { AnalysisTarget, PageKey } from '@/components/workspace/types';
...
export default function ChatPage() {
  ...
  const handleFileChange = useCallback((event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const newFileNames = Array.from(event.target.files).map((file) => file.name);
  ...
  const openAnalysis = useCallback((target: AnalysisTarget) => {
    if (target.sessionId && target.sessionId === analysisSessionId) {
      setPage('analysis');
      return;
    }

    openNewAnalysis(target.prompt);
  }, [analysisSessionId, openNewAnalysis]);
  ...
  }, [analysisSessionId, openNewAnalysis, sendChatMessage]);

  return (
    <div className="flex h-screen min-h-screen bg-white text-slate-900">
      <input
        type="file"
        multiple
        accept=".txt,.md"
      ...
      <div className="flex min-w-0 flex-1 overflow-hidden">
        <div className="relative min-w-0 flex-1 overflow-hidden">
          {page === 'overview' ? <OverviewPage onAction={openAnalysis} instructionPanelOpen={isRightPanelOpen} /> : null}
          {page === 'analysis' ? (
            <AnalysisPage
              busy={analysisBusy}
              messages={messages}
          ...
          {page === 'explore' ? <ExplorePage onAction={openAnalysis} instructionPanelOpen={isRightPanelOpen} /> : null}
        </div>

        <div className="relative shrink-0 overflow-visible">
          <button
            onClick={() => setIsRightPanelOpen((prev) => !prev)}
            className="absolute left-0 top-6 z-20 flex h-11 w-11 -translate-x-1/2 items-center justify-center rounded-full border border-slate-200 bg-white text-slate-400 shadow-[0_8px_24px_-12px_rgba(15,23,42,0.28)] transition-all hover:border-slate-300 hover:text-slate-600"
          ...
          <div
            className={`h-full overflow-hidden border-l border-slate-100 bg-white transition-all duration-300 ease-in-out ${
              isRightPanelOpen ? 'w-[320px] opacity-100 2xl:w-[350px]' : 'w-0 border-l-0 opacity-100'
            }`}
          >
          <div className="flex h-full w-[320px] flex-col overflow-y-auto p-6 2xl:w-[350px]">
            <div className="mb-12">
