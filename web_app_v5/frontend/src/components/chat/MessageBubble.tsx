...
export default function MessageBubble({
  ...
  return (
    <div className="group/msg flex flex-col items-start w-full gap-1 mb-6">
      ...
      {hasProgress ? (
        <ProgressTimeline
          ...
          thinkingByStage={message.thinkingByStage}
        ...
        <div className="flex items-center gap-2.5 py-2">
          ...
          <span className="text-[13px] text-slate-500">Đang kết nối và phân tích yêu cầu…</span>
        </div>
      ) : null}

      {/* Dashboard blocks (ưu tiên) hoặc fallback chart đơn */}
      {hasBlocks && columns && rows ? (
        <BlockRenderer blocks={blocks} columns={columns} rows={rows} instant={message.id.startsWith('history-')} />
      ) : (
      ...
      {strippedContent && (
        <div className="overflow-hidden px-1 py-1 text-[15px] leading-relaxed text-slate-800 animate-in fade-in duration-300">
          <div className="markdown-output prose prose-slate max-w-none prose-p:my-2 prose-headings:mb-3 prose-headings:mt-6 first:prose-headings:mt-0 prose-table:my-4 prose-table:text-sm prose-headings:font-bold prose-headings:text-slate-900 prose-strong:text-slate-900 prose-li:text-slate-900 prose-ol:text-slate-900 prose-ol:marker:text-slate-900 prose-li:marker:font-bold prose-ol:marker:font-bold">
            <MarkdownTableWrapper>
              ...
              >
                {formattedDoneContent || strippedContent}
              </ReactMarkdown>
            </MarkdownTableWrapper>
            {!message.isDone && <span className="animate-pulse text-slate-400">|</span>}
          </div>
          {canCopy && (
            <div className={`flex justify-start w-full transition-opacity duration-200 ${message.isDone ? 'opacity-0 group-hover/msg:opacity-100' : 'opacity-0 pointer-events-none'}`}>
              <ActionButtons copyText={copyText || ''} onRetry={onRetry} />
            </div>
          )}
        </div>
      )}
