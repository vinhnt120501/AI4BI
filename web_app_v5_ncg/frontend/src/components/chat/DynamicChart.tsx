...
function resolveYKeys(data: Row[], xKey: string, yKeys?: string[], singularYKey?: string): string[] {
  if (data.length === 0) return [];
  const availableKeys = Object.keys(data[0]);
  const requested = [...(yKeys || [])];
  if (singularYKey && !requested.includes(singularYKey)) requested.push(singularYKey);

  if (requested.length > 0) {
    const valid = requested.map(k => {
      // 1. Exact match
      if (k in data[0]) return k;
      // 2. Case-insensitive match (crucial for AI consistency)
...
export default function DynamicChart({ block, columns, rows }: DynamicChartProps) {
  ...
  let yKeys = pivotedCategories || resolveYKeys(chartData, resolvedXKey, block.yKeys, (block as any).yKey);
  ...
  if (activeYKeys.length === 0 && !['pie', 'treemap', 'funnel', 'radial_bar', 'sankey', 'sunburst'].includes(chartType)) {
    // If we can't render a chart, fall back to a simple table view instead of returning null
    return (
      <div className="mt-2 overflow-hidden rounded-lg border border-slate-100">
        <div className="bg-slate-50 px-3 py-1.5 text-[10px] font-medium text-slate-500 border-b border-slate-100">
          Không tìm thấy dữ liệu số để vẽ biểu đồ. Đang hiển thị dạng bảng:
        </div>
        <div className="max-h-[250px] overflow-auto">
          <table className="min-w-full text-[12px]">
            <thead>
              <tr className="bg-white">
                {columns.map(c => <th key={c} className="px-3 py-2 text-left font-semibold text-slate-700 border-b">{c}</th>)}
              </tr>
            </thead>
            <tbody>
              {rows.slice(0, 5).map((row, i) => (
                <tr key={i} className="border-b last:border-0">
                  {row.map((val, j) => <td key={j} className="px-3 py-2 text-slate-600">{val}</td>)}
                </tr>
              ))}
            </tbody>
          </table>
  ...
  const yKey = activeYKeys[0] || block.yKey || (block as any).yKey || '';
