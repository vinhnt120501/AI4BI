...
export default function FeedRail({ activeId, onSelect, onSelectKpi, isMcpConfigured }: FeedRailProps) {
  ...
  useEffect(() => {
    if (!isMcpConfigured || signalsStatus !== 'idle') return;
    void loadSignals({ reset: true });
  }, [signalsStatus, loadSignals, isMcpConfigured]);
  ...
  useEffect(() => {
    if (!isMcpConfigured || heartbeatStatus !== 'idle') return;
    void loadHeartbeat({ reset: true });
  }, [heartbeatStatus, loadHeartbeat, isMcpConfigured]);
  ...
  return (
    <aside className="w-[320px] md:w-[360px] shrink-0 overflow-x-hidden border-r border-[color:var(--color-border-tertiary)] bg-[var(--color-background-primary)]">
        ...
        >
          ...
          {!isMcpConfigured ? (
             <div className="px-3 py-2 text-[12px] text-[color:var(--color-text-secondary)]">Chưa có thông tin.</div>
          ) : null}
        ...
        <div className="shrink-0 border-t border-[color:var(--color-border-tertiary)] px-4 pt-5 pb-[calc(16px+env(safe-area-inset-bottom))]">
          ...
          <div className="overflow-x-hidden">
            ...
            {!isMcpConfigured ? (
              <div className="px-1 py-2 text-[12px] text-[color:var(--color-text-secondary)]">Chưa có thông tin.</div>
            ) : null}
