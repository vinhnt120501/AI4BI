...
function buildEventDetail(eventType: string, data: any) {
  ...
  switch (eventType) {
    ...
    case 'qa_result': {
      ...
      const issues = data.issues_count || 0;
      const scores = data.dimension_scores || {};

      // Format dimension scores
      const scoreParts = Object.entries(scores).map(([key, value]) => {
        const score = typeof value === 'number' ? value : 0;
        const label = key.replace(/_/g, ' ').replace(/\b\w/g, c => c.toUpperCase());
        const emoji = score >= 3 ? '🟢' : score >= 2 ? '🟡' : score >= 1 ? '🟠' : '🔴';
        return `${emoji} ${label}: ${score}/3`;
      }).join(' | ');

      const decisionText = decision === 'PASS'
        ? 'Chất lượng tốt, sẵn sàng giao'
        : decision === 'NEEDS_IMPROVEMENT'
        ? `Cần cải thiện (${issues} vấn đề)`
        : decision === 'CRITICAL_ISSUE'
        ? 'Có lỗi nghiêm trọng'
        : 'Đã kiểm tra chất lượng';

      return `QA: ${decisionText} (${(confidence * 100).toFixed(0)}%) ${scoreParts ? '| ' + scoreParts : ''}`;
    }
    case 'timing': {
  ...
  const applySseChunk = useCallback((assistantId: string, rawChunk: string) => {
    ...
    const appendEvent = (message: Message) => {
      ...
      const nextEvent = {
        ...
        raw_data: data,  // Lưu raw data để frontend có thể truy cập
      };
                      ...
                      {files.map((file, index) => (
                        <div key={`${file}-${index}`} className="group flex flex-col gap-1 py-1.5 animate-in fade-in duration-300">
                          <div className="flex items-center justify-between">
                            <span className="mr-2 break-words text-[13px] font-medium text-slate-700">{file}</span>
                            <button
                              onClick={() => setFiles([])}
